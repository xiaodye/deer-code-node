...
import { bashTool, grepTool, lsTool, textEditorTool, todoWriteTool, treeTool } from "../tools/index";
import { CodingAgentState } from "./state";

export function createCodingAgent(pluginTools: StructuredTool[] = []) {
  const model = initChatModel();
  const tools = [bashTool, grepTool, lsTool, textEditorTool, todoWriteTool, treeTool, ...pluginTools];
