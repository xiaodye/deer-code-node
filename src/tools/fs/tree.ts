...
async function generateTree(
  ...
  try {
    ...
    // Filter ignored
    entries = entries.filter((e) => !shouldIgnore(path.join(directory, e.name), e.name, ignorePatterns));
  ...
  },
  {
    ...
    schema: z.object({
      path: z
        .string()
        .optional()
        .describe("Directory path to display. Defaults to current working directory if not specified."),
      max_depth: z
        .number()
        .optional()
        .default(3)
        .describe("Maximum depth to traverse. The max_depth should be less than or equal to 3. Defaults to 3."),
    }),
  }
);
