        ...
        try {
            ...
            if (match && match.length > 0) {
                items = items.filter((item) =>
                    match.some((pattern) => minimatch(item.name, pattern)),
                );
            }
    ...
    },
    {
        ...
        schema: z.object({
            path: z
                .string()
                .describe(
                    'The absolute path to list files and directories from. Relative paths are **not** allowed.',
                ),
            match: z
                .array(z.string())
                .optional()
                .describe('An optional array of glob patterns to match.'),
            ignore: z
                .array(z.string())
                .optional()
                .describe('An optional array of glob patterns to ignore.'),
        }),
    },
);
