...
export const App = () => {
  ...
  const handleSubmit = async (value: string) => {
    ...
    try {
      const stream = await agent.stream({ messages: [...messages, userMsg], todos: todos }, { recursionLimit: 50 });
  ...
  return (
    <Box flexDirection="row" height={30}>
      ...
      <Box width="60%" flexDirection="column">
        {activeTab === 0 ? <TerminalView output={terminalOutput} /> : <TodoListView todos={todos} />}
        <Box marginTop={1}>
